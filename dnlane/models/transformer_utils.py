...
def coordinate_to_encoding(coord_tensor: Tensor,
    ...
    if coord_tensor.size(-1) == 2:
    ...
    elif coord_tensor.size(-1) == 3:
        angle = coord_tensor[...,2] * scale
        pos_angle = angle[...,None] / dim_t
        pos_angle = torch.stack((pos_angle[..., 0::2].sin(), pos_angle[..., 1::2].cos()),
                            dim=-1).flatten(2)   

        pos = torch.cat((pos_y, pos_x, pos_angle),dim=-1)    
        ...
        offset =  coord_tensor[...,3]*scale
        pos_offset = offset[...,None]/dim_t
        pos_offset = torch.stack((pos_offset[..., 0::2].sin(), pos_offset[..., 1::2].cos()),
                            dim=-1).flatten(2)  

        pos = torch.cat((pos_y, pos_x, pos_angle,pos_offset),dim=-1)   
    else:

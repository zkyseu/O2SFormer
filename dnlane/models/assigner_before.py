...
#    predictions = predictions.permute(1,0,2) # prior first
...
#    targets[invalid_masks] = 0
#    predictions[invalid_masks] = 0
#    lengths = (~invalid_masks).sum(dim=-1)
    print('dist:',targets.shape)
    distances = torch.cdist(targets,predictions,p=1)
#    distances = torch.abs((targets - predictions))
#    distances[invalid_masks] = 0.
#    distances = distances / (lengths.float() + 1e-9) # norm
    distances = distances.view(num_priors, batchsize, num_targets)
...
def focal_cost(cls_pred, gt_labels, alpha=0.25, gamma=2, eps=1e-12):
    ...
    """
    gt_labels = gt_labels.permute(1,0)
...
class LaneAssigner(BaseAssigner):
    def assign(self,    
               predictions,
               targets,
               img_w,
        ...
        Args:
            predictions (Tensor): predictions predicted by each stage, shape: (num_layers, num_priors, 78)
        ...
        '''
        predictions = predictions.detach().clone() #[num_layers,num_priors,dim]
#        predictions = predictions.permute(1,0,2)
        predictions[..., 3] *= (img_w - 1)
        predictions[..., 6:] *= (img_w - 1)
        targets = targets.detach().clone()
        targets = targets.unsqueeze(0).repeat(predictions.size(0),1,1) # apply to all layers
        # distances cost
        ...
        prediction_start_xys = prediction_start_xys.permute(1,0,2) #[num_layers,num_priors,dim]
        target_start_xys = target_start_xys.permute(1,0,2)

        start_xys_score = torch.cdist(prediction_start_xys, target_start_xys,p=2).reshape(num_layers, num_priors, num_targets).permute(1,0,2)
        start_xys_score = (1 - start_xys_score / torch.max(start_xys_score)) + 1e-2

        target_thetas = targets[..., 4].permute(1,0).unsqueeze(-1)
        prediction_thetas = predictions[..., 4].permute(1,0).unsqueeze(-1)
        theta_score = torch.cdist(prediction_thetas,target_thetas,p=1).reshape(num_layers, num_priors, num_targets).permute(1,0,2) * 180

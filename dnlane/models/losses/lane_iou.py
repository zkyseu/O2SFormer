...
def liou_loss(pred, target, img_w, length=15,aligned=True):
    return (1 - line_iou(pred, target, img_w, length,aligned=aligned)).mean()

@LOSSES.register_module()
class Line_iou(nn.Module):
    def __init__(self,loss_weight = 1.0,length = 15):
        super().__init__()
        self.length = length
        self.loss_weight = loss_weight
    
    def forward(self,pred, target, img_w,aligned=True):
        return liou_loss(pred, target, img_w, self.length,aligned)*self.loss_weight
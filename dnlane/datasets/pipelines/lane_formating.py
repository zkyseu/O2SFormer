...
from .transform import LaneTransforms

from mmdet.datasets.builder import PIPELINES
from mmdet.datasets.pipelines.formatting import to_tensor,Collect
...
@PIPELINES.register_module()
class GenerateLaneLine(Collect):
    def __init__(self,
                 keys,
                 transforms = None,
                 img_info = None,
        ...
        self.img_w, self.img_h = img_info
        self.num_points = self.n_offsets  = num_points
        ...
        if transforms is None:
            transforms = LaneTransforms(self.img_h, self.img_w)
    ...
    def lane_to_linestrings(self, lanes):
        from imgaug.augmentables.lines import LineString as LS
        lines = []
        for lane in lanes:
            lines.append(LS(lane))
    ...
    def generate_result(self, result):
        img_org = result['img']
        line_strings_org = self.lane_to_linestrings(result['lanes'])
        line_strings_org = LineStringsOnImage(line_strings_org,
                                              shape=img_org.shape)

        for i in range(30):
            if self.training:
                mask_org = SegmentationMapsOnImage(result['mask'],
                                                   shape=img_org.shape)
        ...
        result['img'] = img.astype(np.float32) / 255.
        result['lane_line'] = label
#        print("label shape:",label.shape) #[num_lanes,78]
        result['lanes_endpoints'] = lane_endpoints
        result['gt_points'] = new_anno['lanes']
        result['seg'] = seg.get_arr() if self.training else np.zeros(
            img_org.shape)   
    ...
    def __call__(self, results):
        data = {}
        img_meta = {}
        results = self.generate_result(results)
        for key in self.meta_keys:
            img_meta[key] = results[key]
        data['img_metas'] = DC(img_meta, cpu_only=True)
        for key in self.keys:
